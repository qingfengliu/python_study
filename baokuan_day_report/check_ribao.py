...
to_list = ['qfliu@.com','wguan@.com','wen.xp.sz@belle.com.cn','zyliu@.com','fxwang@.com']
...
def plot_curve1(date,data,title):
    ...
    date.set_index('report_date',inplace=True)
...
cur = conn.cursor()
cur.execute(' SELECT `report_date`,count(*) as `count` FROM .dm_commodity_style_date_report group by `report_date` having TO_DAYS( NOW( ) ) - TO_DAYS( report_date) <= 30;')
temp=cur.fetchall()
cur.execute('SELECT count(*) as `count` FROM .dm_commodity_style_date_report group by `report_date` having TO_DAYS( NOW( ) ) - TO_DAYS( report_date)=1; ')
today=cur.fetchall()
cur.close()
conn.close()
