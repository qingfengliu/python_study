...
to_list = ['xycui@.com','ljwang@.com','wguan@.com']
...
select 
    ...
    from
	(select count(*) as `tm_item` from .jd_staccato_data 
    where `dianpu_name`='耐克（滔搏）品牌授权店' and `dt`='%(time)s') 
    aaa
union 
select 
	'滔搏运动官方旗舰店' as `spidername`,
    ...
    from
	(select count(*) as `Nike_item` from .jd_nike_price_compare 
    where `dt`='%(time)s' and `dianpu_name`='滔搏运动官方旗舰店') 
    aaa
union 
select 
	'耐克（NIKE）京东自营专区' as `spidername`,
	case when `aaa`.`Nike_item`>0  then 1 else 0 end as `spiderstatic`,
    case when `aaa`.`Nike_item`  then `aaa`.`Nike_item` else 0 end as `spideritemcount`
    from
	(select count(*) as `Nike_item` from .jd_nike_price_compare 
    where `dt`='%(time)s' and `dianpu_name`='耐克（NIKE）京东自营专区') 
    aaa
;
...
if return_value==-1:
    ...
    static_str = '爬虫任务检测成功'
if send_mail(to_list, "%s京东三家店铺对比爬虫:%s" %(time.strftime('%Y-%m-%d', time.localtime(time.time())),static_str),content):
    print("发送成功")
else:
    print('发送失败')
exit(return_value)