...
db_insert = web.database(dbn='mysql', db='o2o', user='xxxx', pw='xxxx', port=3306, host='xxxx')
...
class DianpingtuanSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        #     'select distinct shop_id from t_hh_dianping_shop_info_search where search_kw_cate="%s";' % self.search_cate)
        data = db_insert.query('select distinct shop_name,shop_id,city_name from t_hh_dianping_shop_info_search where shop_name like "%星客多%";')
        for d in data:
        ...
        if '页面不存在' in response.body:
            ...
            if detail_list:
                for detail in detail_list:
                    ...
                    item['search_kw_cate'] = self.search_cate
                    # try:
